...
- git push --force -  способ справиться с ошибкой rejected: non-fast-forward. В этом случае команда удалит "лишние" коммиты в удаленной ветке и запишет коммиты из локальной ветки
...
- git merge --no-ff <название ветки> - отключить fast-forward слияние веток
- git merge --no-edit <название ветки> - команда избавляет от необходимости вводить сообщение для merge коммита
- git config [--global] merge.ff false - отключить fast-forward слияние веток навсегда (или пока не вернете настройку как было)
- Если отключить fast-forward слияние веток, то вместо "перемотки" ветки Git создаст в ней коммит слияния (merge commit)
